 ...
 * @returns
 */
export default async function CreateArduinoSketch({ model_chosen, programDescription, apiKey, progressIntervalMs = 1000, onJobCreated, onSuccess, onError, onProgress, print = false, jobId, }) {
    try {
        ...
        const response = await axios.post(`${Constants.api_base_url_tailscale}/create_arduino_sketch?apiKey=${apiKey}`, {
            model_chosen,
            programDescription,
        });
