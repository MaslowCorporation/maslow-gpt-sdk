 ...
 * @returns
 */
export default async function GetChatGPTFunction({ model_chosen, functionName, functionLanguage, functionPurpose, functionArgs, functionReturnValue, apiKey, progressIntervalMs = 1000, onJobCreated, onSuccess, onError, onProgress, print = false, jobId, }) {
    try {
        ...
        const response = await axios.post(`${Constants.api_base_url_tailscale}/get_gpt_func?apiKey=${apiKey}`, {
            model_chosen,
            functionName,
            functionLanguage,
            functionPurpose,
