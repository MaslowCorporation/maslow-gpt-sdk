...
export default async function TransformCloudFile({ publicId, assetType, transformations, deliveryType = 'upload', version, fileExtension, apiKey, progressIntervalMs = 1000, onJobCreated, onSuccess, onError, onProgress, print = false, jobId, }) {
    try {
        ...
        const response = await axios.post(`${Constants.api_base_url_web}/transform_file_cloud?apiKey=${apiKey}`, { publicId, transformations, deliveryType, assetType, version, fileExtension });
